...
#include "module/mks_wifi/mks_wifi_upload.h"
...
uint32_t auto_shutdown = 0;
uint16_t temp_nozzle_0_max = HEATER_0_MAXTEMP;
...
#define BAUDRATE_2 BAUDRATE
    ...
    serial_connect_timeout = millis() + 3000UL;
...
// Set up LEDs early
    ...
    while (find_firmware(&card, ESP_FIRMWARE_FILE) && PENDING(millis(), serial_connect_timeout))
    {
        tft.canvas(0, 0, TFT_WIDTH, TFT_HEIGHT);
        tft.set_background(COLOR_BACKGROUND);
        tft_string.set("wifi uploading...");
        tft.add_text(tft_string.center(TFT_WIDTH), TFT_HEIGHT - 70, COLOR_YELLOW, tft_string);
        tft.queue.sync();

        if (0 == wifi_upload_ESP8266(&card, ESP_FIRMWARE_FILE))
        {
            tft.canvas(0, 0, TFT_WIDTH, TFT_HEIGHT);
            tft.set_background(COLOR_RED);
            tft_string.set("File uploading error...");
            tft.add_text(tft_string.center(TFT_WIDTH), TFT_HEIGHT - 70, COLOR_YELLOW, tft_string);
            tft.queue.sync();
            delay(2000);
        }
    ...
    if (temp_nozzle_max_key == false)
    {
        temp_nozzle_0_max = 290;
        Temperature::hotend_maxtemp[0] = temp_nozzle_0_max;
    }

    MarlinUI::set_status_P(GET_TEXT(WELCOME_MSG), -1);
